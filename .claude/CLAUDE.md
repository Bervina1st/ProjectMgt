...
- **Scope note:** the frontend MVP (`apps/web`) is still **client-side only** — not yet connected to the M1 database, tRPC, auth, worker, or LLM. Those wire in across M1–M3 without restructuring (shared Zod schemas are the contract).

...
- **Scope note:** `apps/web` has **working Supabase magic-link auth — verified end-to-end** (user received link, clicked, landed on `/studio` logged in). `@supabase/supabase-js` **implicit flow** (`flowType: "implicit"`, `detectSessionInUrl: true`) — chosen over PKCE so links work across browsers (PKCE threw "invalid flow state" when opened elsewhere). Client factory in `app/lib/supabase.ts` degrades gracefully if env vars unset; `/signin` + `/signup` call `signInWithOtp`; `/auth/callback` reads the session from the URL; nav shows email + Sign out. Dashboard redirect URL must be allow-listed in Supabase (`http://localhost:3000/**` dev; Netlify URL for prod). Public env vars in `apps/web/.env.local` (gitignored) for local — **must also be set in Netlify** (NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY) or deployed sign-in shows "unconfigured". Still NOT wired: tRPC/DB reads, worker, LLM (land M1–M3).
